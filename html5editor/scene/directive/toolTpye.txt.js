app.directive('drawTxt', ['$document','$compile',function($document,$compile) {

    return {
        link : function(scope,element,attr){
            var template = '<div class="fouce-region" move-component setting="{{txt}}"' +
                            ...
                            'ng-click="chooseComponent(txt.type,txt.id)"   >' +
                                '<div write-content="" contenteditable="true" style="text-align: {{txt.textAlign}};color: {{txt.color}};width: {{setPxToRem(txt.width)}};height:{{setPxToRem(txt.height)}};line-height:{{setPxToRem(txt.lineHeight)}};font-size:{{setPxToRem(txt.fontSize)}};font-style:{{txt.fontStyle}};font-weight:{{txt.fontWeight}};text-decoration:{{txt.textDecoration}};">{{txt.content}}</div>' +
                            '</div>'+
...
app.controller('settingfont', ['$scope', 'dataService', '$http',function($scope, dataService, $http) {
...
//属性修改面板
app.directive('editerTxt', ['$document','$compile',function($document,$compile) {
    return {
...
//todo 输入文本 ==========================
app.directive('writeContent', ['$document','$compile',function($document,$compile) {
